...
public class RoadTile : Tile
{
    ...
    {
        //주변 어느위치에 roadtile이 있는지 표시한 마스크 생성
    ...
    int GetIndex(AdjTilePosition mask)
    {
        switch (mask)
        {
            case AdjTilePosition.None:
                //주변에 roadtile이 없다
                return 0;   //의미 없음(한개만 놓았을 때의 이미지. 가능하면 1자)
...
#if UNITY_EDITOR
    // RoadTile 에셋을 쉽게 만들기 위한 헬퍼 함수
    [MenuItem("Assets/Create/2D/Tiles/RoadTile")]
    public static void CreateRoadTile()
    {
        //파일을 어디다 어떤이름과 확장자로 만들지 결정
        ...
        //위에서 구현 경로를 기반으로 RoadTile 에셋을 하나 만듬
